...
def fetch_live_results():
    ...
    try:
        ...
        print(
            f"[INFO] TheSportsDB-Abruf mit Key {masked} (Liga {WC_LEAGUE_ID}, Saison 2026)."
        )
...
def main():
    ...
    for i, r in result.head(15).iterrows():
        lines.append(
            f"| {i + 1} | {r.Team} | {r.Titel:.1%} | {r.Finale:.1%} | {r.Halbfinale:.1%} |"
        )
    ...
    for _, r in matches.iterrows():
        ...
        lines.append(
            f"| {r['date']} | {r['home_team']}{marker} vs {r['away_team']} | "
    ...
    print(
        f"Ergebnisse: {OUTDIR / 'results.json'} ({len(results_payload)} gespielt, "
